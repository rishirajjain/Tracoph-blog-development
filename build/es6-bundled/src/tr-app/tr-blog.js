...
var f=sessionStorage.flag,idc=doc.id;if("1"===f){let delbtn=document.createElement("paper-button");delbtn.innerHTML="delete";delbtn.setAttribute("id","delbtn");div.appendChild(delbtn);delbtn.onclick=function deletePost(){if(confirm("Are sure you want to delete this post?")){db.collection("blog").doc(idc).delete().then(function(){window.location.reload()}).catch(function(error){window.alert("Failed to delete post: ",error)})}};//update post
let updt=document.createElement("paper-button");updt.innerHTML="update";updt.setAttribute("id","upadtebtn");div.appendChild(updt);var diag=this.$.updatediag;updt.onclick=function updateToggle(){diag.toggle();pid=idc}}//endif
...
window.alert("Failed to update post: ",error)})}addForm(){this.$.dialog.toggle()}subMit(){let title=this.$.title.value,author=this.$.aname.value,content=this.$.content.value,date=new Date().toString();this.fireWrite(title,author,content,date)}//push data into Firebase
