...
<div id="viewFull">
  ...
  var check=sessionStorage['values'];
  var f=sessionStorage['flag'];
  //first load
  if (typeof f==='undefined'){
   
  var values={'addpost':'none',
                'sobtn':'none' 
    }

  window.sessionStorage['values']=JSON.stringify(values);
  var rvalue=JSON.parse(sessionStorage['values'])
  this.$.addpost.style.display = rvalue.addpost;
  this.$.sobtn.style.display   = rvalue.sobtn;
  ...
  this.addEventListener('load', this.fireRead());
  ...
  renderData(doc){
   ...
   let viewpost= document.createElement('paper-button');  

    viewpost.innerHTML= "View Post";
    viewpost.setAttribute('class',"viewpost");
    div.appendChild(viewpost);
    

    
    shw=doc.id;
    var vp= this.$.vipo;
    const full = this.$.viewFull;   
    viewpost.onclick= function viewPost() {
     vp.toggle();
    let div = document.createElement('div'); 
    let title = document.createElement('h2');
    let author = document.createElement('span');
    ...
    var vfp = db.collection("blog").doc(shw);
    ...
    delbtn.onclick= function deletePost(){
    
    if(confirm("Are sure you want to delete this post?")){

    db.collection("blog").doc(idc).delete().then(function() {
      window.location.reload()
    }).catch(function(error) {
    window.alert("Failed to delete post: ",error);
    });
    
    }};

    //update post
    let updt= document.createElement('paper-button');  
    updt.innerHTML= "update";
    updt.setAttribute('class',"upadtebtn");
    div.appendChild(updt);
    var diag=this.$.updatediag;
    
    updt.onclick= function updateToggle(){
     diag.toggle();
     pid=idc;
...
updatePost(){
    var upatedoc = db.collection("blog").doc(pid);

    let title= this.$.uptitle.value;
    let author= this.$.upname.value;
    let content= this.$.upcontent.value;
    ...
    var updatediag=this.$.updatediag; 
    return upatedoc.update(data)
  ...
  window.location.reload()
    ...
    window.alert("Failed to update post: ",error);
    });
...
closed(){
    this.$.vipo.toggle();
    var element = this.shadowRoot.getElementById(chn);
 ...
 subMit(){
    	  ...
    	  let author= this.$.aname.value;
    	  let content= this.$.content.value;
    	  let date= new Date().toString();
    	  this.fireWrite(title,author,content,date);
    };

   //push data into Firebase

 fireWrite(title,author,content,date){
 	let data={
			    title: title,
			    author: author,
			    content: content,
...
readUser(){
	...
	db.collection('userdetail').get().then(snapshot => {
    snapshot.docs.forEach(udat => {
    	this.signIn(udat);
    });
});
}
...
signIn(udat){
	let user= udat.data().username;
	let pass= udat.data().password;
	let userinp= this.$.uname.value;
	let pwdinp=this.$.pwd.value;
	if(user===userinp && pass===pwdinp){

		this.$.login.close();
    var values={'addpost':'block',
                 'sbtn':'none',
                 'sobtn':'block' 
    }
    window.sessionStorage['values']=JSON.stringify(values);
    var rvalue=JSON.parse(sessionStorage['values'])
    this.$.addpost.style.display = rvalue.addpost;
    this.$.sbtn.style.display =rvalue.sbtn;
    this.$.sobtn.style.display=rvalue.sobtn;
...
window.customElements.define('tr-blog', blog);